...
class FinanceRepository{
  ...
  async movimentsByUser(id, type, frequency){
    ...
    console.log('Estou no moviment By User')
    console.log(id, type, frequency)

    const moviments = await mongo.collection('moviments').find(
      {
        user_id: { $eq: id},
        type: { $eq: type},
  ...
  async movimentById(id){
    const moviment = await mongo.collection('moviments').findOne(
    {
